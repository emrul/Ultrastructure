...
public class ModelImpl implements Model {
	...
	public ModelImpl(EntityManagerFactory emf) {
		...
		Workspace kernelWorkspace = KernelUtil
				.getKernelWorkspace(entityManager);
	 ...
	 * 
	 * @see
	 * com.chiralbehaviors.CoRE.meta.Model#deregister(com.chiralbehaviors.CoRE
	 * .workspace.Workspace)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * com.chiralbehaviors.CoRE.meta.Model#find(com.chiralbehaviors.CoRE.attribute
	 * . AttributeValue)
	 */
	@Override
	public <AttributeType extends AttributeValue<RuleForm>, RuleForm extends Ruleform> List<RuleForm> find(
		...
		CriteriaQuery<RuleForm> query = criteriaBuilder
				.createQuery(ruleformClass);
		...
		Root<AttributeValue<?>> attributeValue_ = (Root<AttributeValue<?>>) query
				.from(attributeValue.getClass());

		Predicate whereAttribute = criteriaBuilder
				.equal(attributeValue_.get("attribute"),
						attributeValue.getAttribute());

		@SuppressWarnings("unchecked")
		SingularAttribute<? super AttributeValue<?>, RuleForm> ruleformAttribute = (SingularAttribute<? super AttributeValue<?>, RuleForm>) (SingularAttribute<AttributeType, RuleForm>) attributeValue
				.getRuleformAttribute();
		Predicate whereRuleform = criteriaBuilder.equal(
				attributeValue_.join(ruleformAttribute), rf);

		Predicate whereAttributeValue = null;
		switch (attributeValue.getAttribute().getValueType()) {
		case BINARY: {
			whereAttributeValue = criteriaBuilder.equal(
					attributeValue_.get(AttributeValue_.binaryValue),
					attributeValue.getBinaryValue());
			break;
		}
		case INTEGER: {
			whereAttributeValue = criteriaBuilder.equal(
					attributeValue_.get(AttributeValue_.integerValue),
					attributeValue.getIntegerValue());
			break;
		}
		case BOOLEAN: {
			whereAttributeValue = criteriaBuilder.equal(
					attributeValue_.get(AttributeValue_.booleanValue),
					attributeValue.getBooleanValue());
			break;
		}
		case NUMERIC: {
			whereAttributeValue = criteriaBuilder.equal(
					attributeValue_.get(AttributeValue_.numericValue),
					attributeValue.getNumericValue());
			break;
		}
		case TEXT: {
			whereAttributeValue = criteriaBuilder.equal(
					attributeValue_.get(AttributeValue_.textValue),
					attributeValue.getTextValue());
			break;
		}
		case TIMESTAMP: {
			whereAttributeValue = criteriaBuilder.equal(
					attributeValue_.get(AttributeValue_.timestampValue),
	 ...
	 * (non-Javadoc)
	 * 
	 * @see com.chiralbehaviors.CoRE.meta.Model#find(java.lang.Long,
	 * java.lang.Class)
	 */
	@Override
	...
	@SuppressWarnings("unchecked")
	@Override
	public <RuleForm extends ExistentialRuleform<?, ?>> RuleForm find(
			String name, Class<RuleForm> ruleform) {
		try {
			return (RuleForm) em
					.createNamedQuery(prefixFor(ruleform) + FIND_BY_NAME_SUFFIX)
	 ...
	 */
	@Override
	public <RuleForm extends Ruleform> List<RuleForm> findFlagged(
	 ...
	 * 
	 * @see
	 * com.chiralbehaviors.CoRE.meta.Model#findUpdatedBy(com.chiralbehaviors
	 * .CoRE.agency .Agency, java.lang.Class)
	 */
	@SuppressWarnings("unchecked")
	@Override
	public <RuleForm extends Ruleform> List<RuleForm> findUpdatedBy(
			Agency updatedBy, Class<Ruleform> ruleform) {
		return em.createNamedQuery(prefixFor(ruleform) + FIND_BY_NAME_SUFFIX)
				.setParameter("agency", updatedBy).getResultList();
	}
	 ...
	 * 
	 * @see
	 * com.chiralbehaviors.CoRE.meta.Model#register(com.chiralbehaviors.CoRE
	 * .workspace.Workspace)
	 */
	@Override
