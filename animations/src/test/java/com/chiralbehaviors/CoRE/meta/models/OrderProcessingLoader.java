...
public class OrderProcessingLoader extends OrderProcessingWorkspace {
	...
	public void createMetaProtocols() {
		MetaProtocol m1 = model.getJobModel().newInitializedMetaProtocol(
				deliver, core);
		...
		MetaProtocol m6 = model.getJobModel().newInitializedMetaProtocol(
				printPurchaseOrder, core);
	...
	public void createProtocols() {
		...
		Protocol pickProtocol = model.getJobModel().newInitializedProtocol(
				deliver, core);
		...
		Protocol shipProtocol = model.getJobModel().newInitializedProtocol(
				deliver, core);
		...
		Protocol printPoProtocol = model.getJobModel().newInitializedProtocol(
				ship, core);
		...
		Protocol feeProtocol = model.getJobModel().newInitializedProtocol(
				printPurchaseOrder, core);
		...
		Protocol salesTaxProtocol = model.getJobModel().newInitializedProtocol(
				fee, core);
		...
		Protocol discountProtocol = model.getJobModel().newInitializedProtocol(
				fee, core);
	...
	public void createStatusCodes() {
		...
		completed.setPropagateChildren(true); // This is done to test the dup
												// logic in the job animation
