...
public class AbstractNetworkModelTest extends AbstractModelTest {
	...
	public void testGetImmediateRelationships() throws Exception {
		...
		em.flush();
		em.refresh(a);
		assertEquals(1, model.getAgencyModel().getImmediateRelationships(a)
				.size());
	}
	...
	public void testGetTransitiveRelationships() throws SQLException {
		...
		em.refresh(b);
		em.refresh(c);
		assertEquals(2, model.getAgencyModel().getTransitiveRelationships(a)
				.size());
	}
	...
	public void testInGroup() {
		...
		Relationship classification = new Relationship(
				"test group classification", kernel.getCore());
		em.persist(classification);
		Relationship inverse = new Relationship(
				"inverse test group classification", kernel.getCore());
		em.persist(inverse);
		...
		Agency testAgency = model.getAgencyModel()
				.create("test agency in group", "test", myAspect).asRuleform();
	...
	public void testNotInGroup() {
		em.getTransaction().begin();
		Relationship classification = new Relationship(
				"test not in group classification", kernel.getCore());
		em.persist(classification);
		Relationship inverse = new Relationship(
				"inverse test not in group classification", kernel.getCore());
		em.persist(inverse);
		classification.setInverse(inverse);
		inverse.setInverse(classification);
		...
		em.persist(classifier);
		em.getTransaction().commit();
		List<Agency> notInGroup = model.getAgencyModel().getNotInGroup(
				classifier, inverse);
