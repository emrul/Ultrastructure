...
public class JobModelTest extends AbstractModelTest {
	...
	public static void before() throws Exception {
		...
		loader.load();
		scenario = loader.createWorkspace(em)
	...
	public void testEuOrder() throws Exception {
		...
		EntityTransaction txn = em.getTransaction();
		txn.begin();
		Job order = model.getJobModel().newInitializedJob(
				scenario.getDeliver(), kernel.getCore());
	...
	public void testGenerateJobsFromProtocols() throws Exception {
		...
		TestDebuggingUtil.printMetaProtocolGaps(jobModel
				.findMetaProtocolGaps(order));
		List<Protocol> protocols = model.getJobModel().getProtocolsFor(
				order.getService());
	...
	public void testGetActiveJobs() throws Exception {
		...
		List<StatusCode> states = Arrays.asList(scenario.getActive(),
				scenario.getAvailable(), scenario.getAbandoned());
		List<Job> active = jobModel.getActiveJobsFor(
				scenario.getOrderFullfillment(), states);
	...
	public void testIteration() throws Exception {
		...
		em.persist(child3);
		Relationship childRelationship = model.getRelationshipModel().create(
				"child of", "test relationship", "parentOf",
		...
		sequencings.add(new Tuple<StatusCode, StatusCode>(scenario
				.getAvailable(), scenario.getCompleted()));
		...
		model.getJobModel().createStatusCodeSequencings(childService,
				sequencings, kernel.getCore());

		ProductChildSequencingAuthorization auth = new ProductChildSequencingAuthorization(
				service, scenario.getAvailable(), childService,
				scenario.getAvailable(), kernel.getCore());
		em.persist(auth);
		Protocol p = jobModel.newInitializedProtocol(service, kernel.getCore());
	...
	public void testJobGenerationAndSequencing() throws Exception {
		...
		em.persist(p);
		model.getJobModel().createStatusCodeChain(
				pushit,
				new StatusCode[] { pushingMe, shovingMe,
						scenario.getCompleted() }, kernel.getCore());

		ProductSelfSequencingAuthorization auth = new ProductSelfSequencingAuthorization(
				pushit, pushingMe, shovingMe, kernel.getCore());
		em.persist(auth);
		em.getTransaction().commit();
		em.getTransaction().begin();
	...
	public void testMetaProtocols() throws Exception {
		...
		job = model.getJobModel().newInitializedJob(
				scenario.getPrintPurchaseOrder(), kernel.getCore());
		job.setAssignTo(scenario.getOrderFullfillment());
		job.setProduct(scenario.getABC486());
		job.setDeliverTo(scenario.getRSB225());
		job.setDeliverFrom(scenario.getFactory1());
	...
	public void testOrder() throws Exception {
		...
		List<Job> pickSiblings = jobModel.getActiveSubJobsForService(
				pick.getParent(), scenario.getShip());
	...
	public void testSelfSequencingAuthorization() {
		...
		StatusCodeSequencing sequence = new StatusCodeSequencing(service,
				kickingAss, takingNames, kernel.getCore());
		em.persist(sequence);

		ProductSelfSequencingAuthorization auth = new ProductSelfSequencingAuthorization(
				service, kickingAss, takingNames, kernel.getCore());
		em.persist(auth);
		em.getTransaction().commit();
		em.getTransaction().begin();
	...
	public void testTerminateChildrenParent() throws IOException {
		...
		model.getJobModel().createStatusCodeChain(pushit,
				new StatusCode[] { pushingMe, shovingMe }, kernel.getCore());
		...
		ProductChildSequencingAuthorization auth = new ProductChildSequencingAuthorization(
				shoveit, shovingMe, pullit, shovingMe, kernel.getCore());
		em.persist(auth);
		ProductParentSequencingAuthorization auth2 = new ProductParentSequencingAuthorization(
				shoveit, shovingMe, pushit, shovingMe, kernel.getCore());
		em.persist(auth2);
		em.getTransaction().commit();
		em.getTransaction().begin();
		...
		Job shovingJob = model.getJobModel()
				.getActiveSubJobsForService(push, shoveit).get(0);
		em.getTransaction().begin();
	...
	private List<String> verifyChronologyFields(Job job,
		...
		for (String field : fieldsToMatch) {
			ExistentialRuleform<?, ?> jobRf = (ExistentialRuleform<?, ?>) PropertyUtils
					.getSimpleProperty(job, field);
			System.out.println(String.format("Job %s: %s", field, jobRf));
			ExistentialRuleform<?, ?> chronoRf = (ExistentialRuleform<?, ?>) PropertyUtils
					.getSimpleProperty(jobChronology, field);
			...
			if (!chronoRf.equals(jobRf)) {
				System.out.println(String.format("%s: job: %s, chronology: %s",
						field, jobRf == null ? "null" : jobRf.getName(),
						chronoRf == null ? "null" : chronoRf.getName()));
